...
::: warning
На момент написания статьи, панель версии v3.2.8 **не поддерживает** настройку лимита IP-адресов для клиентов напрямую через панель. В разделе [Настройка лимита IP-адресов](#настройка-лимита-ip-адресов) описан процесс настройки, однако он рекомендуется лишь для продвинутых пользователей.
:::

#### Настройка лимита IP-адресов
::: warning
Рекомендуется создать резервную копию базы данных панели перед выполнением действий в инструментах разработчика. Неверные изменения тела запросов могут нарушить учёт клиентов. Пример создания копии базы данных:
...
1. Подключитесь к вашему серверу по SSH либо через VNC.
2. Войдите в меню `x-ui`.
3. Выберите пункт "IP Limit Management" (пункт 21).
4. Выберите "Install Fail2ban and configure IP Limit" (пункт 1) и подтвердите установку, нажав Enter.

Если установка прошла успешно, вы увидите сообщение об успешной установке:
![successfull fail2ban installation](/images/vpn/3x-ui/ip_limit_install_success.png)

5. Перезагрузите панель (пункт 13).
6. Откройте панель 3X-UI в браузере и перейдите в список клиентов.
7. Нажмите "Edit" у нужного клиента, откройте инструменты разработчика (ПКМ → Inspect) и перейдите на вкладку "Network".
8. Нажмите "Save" — в списке сетевых запросов появится POST‑запрос на путь `<panel url>/panel/api/clients/update/<email>`.
9. Кликните правой кнопкой по запросу и выберите "Edit and Resend".
10. В теле запроса найдите поле `limitIp`, измените его на необходимое значение и отправьте запрос (Send).
![ip limit body](/images/vpn/3x-ui/ip_limit_body.png)
11. Проверьте значение "IP Limit" в информации клиента.
